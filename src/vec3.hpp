...
inline vec3 refract(const vec3& incoming, const vec3& normal, double refrac_index_ratio) {
    double cos_theta = std::fmin(dot(-incoming, normal), 1.0);
    vec3 fractured_ray_perp = refrac_index_ratio * (incoming + cos_theta * normal);
    vec3 fractured_ray_parallel = -std::sqrt(std::fabs(1.0 - fractured_ray_perp.length_squared())) * normal;
    return fractured_ray_parallel + fractured_ray_perp;
}
